...
function compareRows(tDUTrow, tCurColorSensor, lux_check_enable)
	...
	if tDUTrow ~= nil then 
		...
		or		  tCurColorSensor.sat >= tDUTrow.sat + tDUTrow.tol_sast)) then 
			return 3, string.format("Wrong LED Color!\n			Want: %s with %3d nm\n 			Detected: %s with %3d nm", 
					tReferenceLEDcolors[tDUTrow.nm].colorname, tReferenceLEDcolors[tDUTrow.nm].nm, 
					tReferenceLEDcolors[tCurColorSensor.nm].colorname, tReferenceLEDcolors[tCurColorSensor.nm].nm), dnm, dsat, dlux
		
		-- Neither saturation nor wavelength fits --
		else 
			return 1, string.format("Both saturation and wavelength do not fit!", tReferenceLEDcolors[tDUTrow.nm].colorname,
					tReferenceLEDcolors[tDUTrow.nm].nm), dnm, dsat, dlux
		end 
	
	-- the row field is nil thus we do not need a test 
	else 
		return 4, "NO TEST ENTRY", 0, 0, 0
	end 
