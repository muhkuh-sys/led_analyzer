...
void ColorControlFrame::OnGenerateTest(wxCommandEvent& event)
{
    ...
    /* Check test tables for plausibility */
    if(!this->TestEntriesOK())
    {
...
void ColorControlFrame::GenerateNetXTestTable(wxTextFile* tFile)
{
    ...
    int iDevCounter = 0;
    int iIndexLastEntry   = this->GetLastEntry();

    for(int i = 0; i < m_sensorPanels.size(); i++)
    {

        if(m_sensorPanels.at(i)->GetVectorTestrow().size() > 0)
        {
            if( i == iIndexLastEntry) tFile->AddLine(m_sensorPanels.at(i)->GetAtPinsUnderTest(true));

            else  tFile->AddLine(m_sensorPanels.at(i)->GetAtPinsUnderTest(false));
        }
...
int ColorControlFrame::GetLastEntry()
{
    int iLastEntry = -1;
    ...
    for(int i = 0; i<m_sensorPanels.size(); i++)
    {
        if( m_sensorPanels.at(i)->GetVectorTestrow().size() > 0) iLastEntry = i;
    }

    return iLastEntry;
}

bool ColorControlFrame::TestEntriesOK()
{
  ...
  m_text->SetDefaultStyle(wxTextAttr(MYORANGE));

  for(wxVector<PanelSensor*>::iterator it_sensor = m_sensorPanels.begin(); it_sensor != m_sensorPanels.end(); it_sensor++)
  {
...
void ColorControlFrame::InsertHeaders(wxTextFile* tFile)
{
...
void ColorControlFrame::GenerateTestStepFunctions(wxTextFile* tFile)
{
    int iMaxNumber = this->GetMaximumNumberOfTestsets();
